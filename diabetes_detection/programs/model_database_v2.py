...
def pipeline_loader(model):
    return make_pipeline(RobustScaler(), model)

ridgec = pipeline_loader(RidgeClassifierCV(alphas=alphas_alt, cv=kfolds))


lr_elasticnet = pipeline_loader(LogisticRegression(penalty = 'elasticnet',
                                                    solver = 'saga',
                                                    max_iter=1e7,
                                                    l1_ratio=0.05, # 0.04
...
svc = pipeline_loader(SVC(C=20,
                            gamma=0.0003, # 0.0003
                            random_state=controler.rndm_state))


gbc = pipeline_loader(GradientBoostingClassifier(learning_rate=0.01, #0.05
                                                    max_depth=4,
                                                    max_features='sqrt',
...
lightgbmc = pipeline_loader(LGBMClassifier(objective='binary',
                                            num_leaves=4,
...
xgboostc = pipeline_loader(XGBClassifier(learning_rate=0.01, # 0.01
                                            max_depth=3,
                                            min_child_weight=0,
                                            gamma=0.0000, # 0
                                            subsample=0.7,
...
lr = LogisticRegression = pipeline_loader(LogisticRegression(**param1))

param2 = {'n_neighbors': 4, 'metric': 'minkowski', 'p':2}
knn = KNeighborsClassifier = pipeline_loader(KNeighborsClassifier(**param2))
...
param4 = {'criterion': 'gini', 'max_depth': 3, 'max_features': 2, 'min_samples_leaf': 3,
                                                'random_state': controler.rndm_state}
dt = DecisionTreeClassifier = pipeline_loader(DecisionTreeClassifier(**param4))

#param5 = {'learning_rate': 0.05, 'n_estimators': 150}
param5 = {'learning_rate': 0.04, 'n_estimators': 150}
adab = AdaBoostClassifier = pipeline_loader(AdaBoostClassifier(**param5))

param6 = {'learning_rate': 0.01, 'n_estimators': 100}
gbc2 = GradientBoostingClassifier = pipeline_loader(GradientBoostingClassifier(**param6))

gnb = GaussianNB = pipeline_loader(GaussianNB())

param8 = {'n_estimators': 15, 'criterion': 'gini', 'random_state':controler.rndm_state}
rf = RandomForestClassifier = pipeline_loader(RandomForestClassifier(**param8))

et = ExtraTreesClassifier = pipeline_loader(ExtraTreesClassifier())
